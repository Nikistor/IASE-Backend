...
urlpatterns = [
    ...
    path(
        "api/companies/<int:company_id>/add_to_requisition/", add_company_to_requisition
    ),  # POST
    ...
    path(
        "api/requisitions/<int:requisition_id>/update/", update_requisition
    ),  # PUT/PATCH
    path(
        "api/requisitions/<int:requisition_id>/update_bankrupt/",
        update_requisition_bankrupt,
    ),  # POST
    path(
        "api/requisitions/<int:requisition_id>/update_status_user/", update_status_user
    ),  # PUT
    path(
        "api/requisitions/<int:requisition_id>/update_status_admin/",
        update_status_admin,
    ),  # PUT
    path("api/requisitions/<int:requisition_id>/delete/", delete_requisition),  # DELETE
    path(
        "api/requisitions/<int:requisition_id>/delete_company/<int:company_id>/",
        delete_company_from_requisition,
    ),  # DELETE
    path(
        "api/requisitions/<int:requisition_id>/download_report/",
        download_requisition_report,
    ),  # GET
